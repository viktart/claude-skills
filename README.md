...
Scaffolding asks which simulator to run tests against (never assumes one) and writes a `Fastfile`/`Appfile` and a `.github/workflows/ios-release.yml` (manual `workflow_dispatch`). Code signing uses Apple's cloud-managed certificates (`xcodebuild` cloud signing via the App Store Connect API key) — there is no certs repo, no `fastlane match`, and no local certificate to manage. The 3 required secrets are handled by a single command you run yourself, directly in your terminal: `bundle exec fastlane setup_secrets`. That lane collects the ASC API key interactively (the one credential Apple requires creating through their web UI, so nothing secret gets typed into the conversation) and pushes the secrets to GitHub in one go. The release lanes themselves check the secrets are present before doing anything and point back at `setup_secrets` if they aren't, instead of failing with a cryptic signing error.
